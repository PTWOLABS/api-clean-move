...
@Controller("/customers/:customerId/profile-image")
@Roles(["ESTABLISHMENT"])
export class UploadCustomerProfileImageController {
  ...
  @ApiOperation({ summary: "Upload profile image for a customer." })
  ...
  @ApiBadRequestResponse({ description: "Invalid customer id or image file." })
  @ApiUnauthorizedResponse({ description: "Missing or invalid access token." })
  @ApiForbiddenResponse({ description: "Operation not allowed." })
  @ApiNotFoundResponse({ description: "Establishment or customer not found." })
  @ApiInternalServerErrorResponse({
  ...
  async handle(
    ...
    const result = await this.uploadDomainImage.execute({
      ...
      entityId: parsedCustomerId.data,
      file: uploadFile,
    });
