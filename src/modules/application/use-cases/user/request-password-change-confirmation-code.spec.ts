...
describe("Request password change confirmation code", () => {
  ...
  it("should send a confirmation code for OAuth-only user setting first password", async () => {
    ...
    expect(
      inMemoryConfirmationCodesRepository.items[0]?.pendingPasswordHash,
    ).toBe("first-local-password-hashed");
  ...
  it("should remove stored code when email delivery fails", async () => {
    const user = makeUser("CUSTOMER", {
      email: new Email("oauth@example.com"),
      hashedPassword: null,
    });
    ...
    const failingSut = new RequestPasswordChangeConfirmationCodeUseCase(
      inMemoryUsersRepository,
      inMemoryConfirmationCodesRepository,
      fakeConfirmationCodeGenerator,
      fakeTokenHasher,
